...
class VGGTActionQueryModule(nn.Module):
    ...
    def forward(self, vggt_layers, patch_start_idx):
        ...
        per_layer_outputs = []
        num_iters = len(vggt_layers) // self.stride
        for i, vggt_idx in enumerate(range(0, len(vggt_layers), self.stride)):
            vggt_kv = vggt_layers[vggt_idx].squeeze(1)[:, patch_start_idx:]
            queries = self.ca_layers[i * self.num_ca_layers // num_iters](queries, self.vggt_proj(vggt_kv))
            per_layer_outputs.append(queries)

        stacked = torch.stack(per_layer_outputs, dim=1)
        stacked = stacked.repeat_interleave(self.stride, dim=1)[:, :self.num_feature_layers]
        return stacked