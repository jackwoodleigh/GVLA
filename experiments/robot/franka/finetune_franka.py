...
LoRA fine-tuning of VLA-Adapter (LIBERO-Spatial-Pro) on collected Franka demo episodes.

Episode data layout (episodes_bowl_pickplace/episode_*/):
  state/pose_wrt_world.npy   (T, 7) — xyz + quat(xyzw) in robot base frame
  state/grasp.npy            (T,)   — binary 0=closed 1=open
  state/gripper_qpos.npy     (T,)   — scalar [0,1]
  cam3/rgb/*.png             scene camera frames (filename = ns timestamp → sorted = chronological)
  cam4/rgb/*.png             wrist camera frames

Actions are computed as delta poses expressed in LIBERO OSC units, then normalized
with the same q01/q99 stats the base model was trained on (libero_spatial_no_noops).
This keeps the action space compatible so the eval script works without modification.

Run with:
    sbatch experiments/robot/franka/run_finetune.sh
"""
...
class FinetuneConfig:
    # --- Model ---
    pretrained_checkpoint: str = "output/LIBERO-Spatial-Pro"
    ...
    # --- Output ---
    output_dir: str = "output/LIBERO-Spatial-Pro-FrankaFT"
...
def _compute_actions_osc(pose: np.ndarray, grasp: np.ndarray) -> np.ndarray:
    """
    Compute (T, 7) action array in LIBERO OSC units from consecutive EEF poses.

    Translation: delta_xyz (m) / OSC_TRANSLATION_SCALE
    Rotation:    relative axis-angle (rad) / OSC_ROTATION_SCALE
    Gripper:     grasp (0=closed, 1=open) passed through unchanged

    The last timestep is padded by repeating the previous action since there
    is no t+1 to diff against.
    """
    T = len(pose)
...
def _normalize_action(action_osc: np.ndarray, norm_stats: Dict) -> np.ndarray:
    """
    Normalize a 7-dim OSC action to the [-1, 1] range used by the model.

    Dims 0-5 (pose) are normalized via q01/q99 and clipped.
    Dim 6 (gripper) has mask=False so it is passed through as 0.0 or 1.0.
    """
...
class FrankaEpisodeDataset(Dataset):
    """
    PyTorch Dataset over collected Franka demo episodes.

    Each sample corresponds to one timestep t in one episode and contains:
      - scene + wrist images at t
      - normalized proprio at t
      - normalized action chunk [t, t + chunk_size)
      - tokenized prompt + action tokens (minivlm format)
    """
    ...
    def __init__(
        ...
        # Build (episode_path, start_timestep) index and pre-load state data
        ...
        for ep_path in sorted(Path(episodes_dir).iterdir()):
            ...
            # Only start indices where a full chunk fits
            for t in range(T - chunk_size):
                self.samples.append((ep_path, t))

        print(
            f"FrankaEpisodeDataset: {len(self.samples)} samples "
            f"from {len(self._ep_cache)} episodes"
        )

    def _build_tokens(self, actions_chunk_norm: np.ndarray):
        """
        Build input_ids and labels in minivlm format.

        Mirrors RLDSBatchTransform.__call__ (use_minivlm=True branch).
        The last NUM_TOKENS positions of labels hold action token IDs
        (> ACTION_TOKEN_BEGIN_IDX); everything before is IGNORE_INDEX.
        """
        ...
        # Remove the 3 trailing tokens Qwen appends (same offset as RLDSBatchTransform)
        if len(input_ids) >= 3:
            del input_ids[-1]
            del input_ids[-1]
            del input_ids[-1]

        # Discretize actions → token IDs (all > ACTION_TOKEN_BEGIN_IDX)
        ...
        # Pad or truncate to exactly NUM_TOKENS action tokens
        ...
        # Mask everything except the last NUM_TOKENS + 1 positions (actions + stop token)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Action chunk: shape (chunk_size, ACTION_DIM), values in model's normalized space
        ...
        return {
            ...
            "input_ids": input_ids,
            "labels": labels,
            "actions": actions_chunk,       # (chunk_size, ACTION_DIM) float32
            "proprio": proprio,             # (PROPRIO_DIM,) float32
...
def train(cfg: FinetuneConfig):
    ...
    # --- Load normalization stats from base checkpoint ---
    ...
    # --- Load base VLA (fully merged weights from LIBERO-Spatial-Pro) ---
    print("Loading base VLA checkpoint...")
    ...
    # Capture architecture constants before LoRA wrapping changes attribute paths
    llm_dim = vla.llm_dim
    num_patches = (
    ...
    # --- Load action head and proprio projector from base checkpoint ---
    ...
    # --- Optimizer: LoRA params + action_head + proprio_projector ---
    ...
    print(f"\nStarting training: max_steps={cfg.max_steps}, batch_size={cfg.batch_size}")
    print(f"Dataset size: {len(dataset)} samples  |  Steps per epoch: {len(dataloader)}\n")

    while step < cfg.max_steps:
        # Cycle the dataloader since dataset is tiny
        try:
        ...
        # Extract action-position hidden states (mirrors run_forward_pass in finetune.py)
        ...
        for layer_hidden in output.hidden_states:
            ...
            # Gather action positions and reshape to (B, 1, NUM_TOKENS, D)
            action_hidden = (
                text_hidden[action_mask]
                .reshape(batch_size, 1, NUM_TOKENS, -1)
                .to(torch.bfloat16)
            )
            # Task context: vision patch hidden states (B, 1, num_patches, D)
            task_hidden = layer_hidden[:, :num_patches].reshape(batch_size, 1, num_patches, -1)
        ...
        if step % cfg.save_freq == 0 or step == cfg.max_steps:
            _save_checkpoint(cfg, vla, action_head, proprio_projector, processor, step, device)

    print("\nTraining complete.")


def _save_checkpoint(cfg, vla, action_head, proprio_projector, processor, step, device):
    """
    Merge LoRA weights into the base model and save a checkpoint compatible
    with eval_open_loop.py (same layout as LIBERO-Spatial-Pro).
    """
    ...
    # Merge LoRA into base weights so eval can load with from_pretrained directly.
    # deepcopy so the live training model retains its LoRA adapters.
    merged_vla = copy.deepcopy(vla).merge_and_unload()
    merged_vla.save_pretrained(str(out_dir))
    processor.save_pretrained(str(out_dir))

    # Save action head and proprio projector
    torch.save(
        action_head.state_dict(),
        out_dir / "action_head--checkpoint.pt",
    )
    torch.save(
        proprio_projector.state_dict(),
        out_dir / "proprio_projector--checkpoint.pt",
    )

    # Copy dataset statistics so eval can find the unnorm_key
    src_stats = Path(cfg.pretrained_checkpoint) / "dataset_statistics.json"
    shutil.copy2(src_stats, out_dir / "dataset_statistics.json")
